...
namespace Enterprises.Framework.Utility
{
    /*
     * Base64编码规则：
     *s 由基 64 数字、空白字符和尾随填充字符组成。
         从零开始以升序排列的以 64 为基的数字为大写字符“A”到“Z”、小写字符“a”到“z”、数字“0”到“9”以及符号“+”和“/”。 
    ...
    public class EncodingUtility
    {
        ...
        {
            return EncodingUrl(rawUrl, Encoding.GetEncoding("gb2312"));
        }
        ...
        {
            return DeEncodeUrl(rawUrl, Encoding.GetEncoding("gb2312"));
        }
        ...
        {
            return System.Web.HttpUtility.UrlEncode(rawUrl, e);
        }        

        /// <summary>
        /// 采用指定的解码方式对Url字符串进行解码
        /// </summary>
        /// <param name="rawUrl"></param>
        /// <param name="e"></param>
        /// <returns></returns>
        ...
        {
            return System.Web.HttpUtility.UrlDecode(rawUrl, e);
        }

        /// <summary>
        /// 把ASCII字符串转换为byte[]数组
        /// </summary>
        /// <param name="asciiString">源ASCII字符串</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// 把正常的ASCII字符串转换为以base64为基础编码的字符串
        /// </summary>
        /// <param name="srcAsciiString"></param>
        /// <returns></returns>
        public static string ToBase64FromAsciiString(string srcAsciiString)
        {
            byte[] bs = ToBytesFromAsciiString(srcAsciiString);
            return Convert.ToBase64String(bs);
        }

        /// <summary>
        /// 把以base64为基础编码的字符串转换为正常的ASCII字符串
        /// </summary>
        ...
        public static string ToAsciiStringFromBase64(string base64String)
        {
            ...
            return ToAsciiStringFromBytes(bs);
        }

        /// <summary>
        /// 根据指定的编码方式把Base64转换到Byte[]数组
        /// </summary>
        /// <param name="base64String"></param>
        /// <returns></returns>
        ...
        /// GB2312字符串转换为字节数组
        /// </summary>
        /// <param name="srcGb2312String"></param>
        /// <returns></returns>
        ...
        {
            byte[] bs = ToBytesFromUnicode(srcUnicodeString);
            return Convert.ToBase64String(bs);
        }
        ...
        {
            byte[] bs = ToBytesFromDefaultEncoding(srcUnicodeString);
            return Convert.ToBase64String(bs);
        }

        /// <summary>
        /// 把以Gb2312的字符串转换为Base64的GB2312字符串
        /// </summary>
        /// <param name="srcGb2312String"></param>
        /// <returns></returns>
        public static string ToBase64FromGb2312(string srcGb2312String)
        {
            byte[] bs = ToBytesFromGb2312(srcGb2312String);
            return Convert.ToBase64String(bs);
        }

        /// <summary>
        /// 把以base64为基础编码的字符串转换为正常的GB2312字符串
        /// </summary>
        /// <param name="base64String"></param>
        /// <returns></returns>
        ...
        {
            byte[] bs = Convert.FromBase64String(base64String);
            return ToGb2312FromBytes(bs);
        }

        /// <summary>
        /// 把以base64为基础编码的字符串转换为正常的Unicode字符串
        /// </summary>
        /// <param name="base64String"></param>
        /// <returns></returns>
        ...
        {
            byte[] bs = Convert.FromBase64String(base64String);
            return ToUnicodeFromBytes(bs);
        }

        // <summary>
        /// 把以base64为基础编码的字符串转换为操作系统默认编码格式的字符串
        /// 
        /// <param name="base64String"></param>
        /// <returns></returns>
        ...
        {
            byte[] bs = Convert.FromBase64String(base64String);
            return ToDefaultEncodingFromBytes(bs);
        }
        ...
        {
            Encoding gb = Encoding.GetEncoding(charset);
            ...
            foreach (char c in normalString)
            {
                ...
                {
                    byte[] bs = gb.GetBytes(c.ToString(CultureInfo.InvariantCulture));
                    string left = Convert.ToString(bs[0], 16).ToUpper();
                    string right = Convert.ToString(bs[1], 16).ToUpper();
        ...
        public static string Qp2String(string qpString, string charset)
        {
            ...
            for (int i = 0; i < qpString.Length; i++)
            {
                if (qpString[i] == '=')
                {
                    string left = qpString[i + 1].ToString(CultureInfo.InvariantCulture) + qpString[i + 2].ToString(CultureInfo.InvariantCulture);
                    string right = qpString[i + 4].ToString(CultureInfo.InvariantCulture) + qpString[i + 5].ToString(CultureInfo.InvariantCulture);
                    byte b1 = Convert.ToByte(left, 16);
                    byte b2 = Convert.ToByte(right, 16);
                    sb.Append(gb.GetString((new[] { b1, b2 })));
                    i += 5;
                }
                else
                {
                    sb.Append(qpString[i]);
                }
            }
            return sb.ToString();
        }
        ...
        public static string GetFileEncodingName(string fullFileName)
        {
            ...
            string encodingName = IdentifyEncoding.GetEncodingString(fi);
            return encodingName;
        }
        ...
        {
            string encodingName = IdentifyEncoding.GetEncodingString(new Uri(urlAddress));
            return encodingName;
        }
        ...
        {
            string encodingName = IdentifyEncoding.GetEncodingString(input);
            return encodingName;
        }
        ...
        /// 检测字符编码的类
        /// </summary>
        private class IdentifyEncoding
        {
            #region Fields.....
            ...
            /// </summary>
            static IdentifyEncoding() { Initialize_Frequencies(); }
            ...
            /// </code>
            /// </example>
            public static string GetEncodingString(System.Uri testUrl)
            {
                byte[] rawtext = new byte[1024];
                int bytesread = 0, byteoffset = 0;
                using (Stream fsFile = WebRequest.Create(testUrl.AbsoluteUri).GetResponse().GetResponseStream())
                {
                    while ((bytesread = ReadInput(fsFile, ref rawtext, byteoffset, rawtext.Length - byteoffset)) > 0)
                    {
            ...
            /// <param name="fi"></param>
            /// <returns></returns>
            public static string GetEncodingString(System.IO.FileInfo fi)
            {
                byte[] rawtext;
                rawtext = new byte[(int)FileLength(fi)];
                using (FileStream fsFile = new FileStream(fi.FullName, FileMode.Open, FileAccess.Read))
                {
                    ReadInput(fsFile, ref rawtext, 0, rawtext.Length);
                }
                return GetEncodingString(rawtext);
            }
            ...
            /// <returns></returns>
            public static string GetEncodingString(byte[] rawtext)
            {
                int[] scores;
            ...
            /// 判断是GB2312编码的可能性
            /// </summary>
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int GB2312Probability(byte[] rawtext)
            {
                int i, rawtextlen = 0;
                ...
                for (i = 0; i < rawtextlen - 1; i++)
                {
                    ...
                    {
                        dbchars++;
                        if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <= (byte)Identity(0xF7) && (byte)Identity(0xA1) <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))
                        {
                            gbchars++;
                            totalfreq += 500;
            ...
            private static int GBKProbability(byte[] rawtext)
            {
                ...
                for (i = 0; i < rawtextlen - 1; i++)
                {
                    ...
                    else
                    {
                        ...
                        else if ((byte)Identity(0x81) <= rawtext[i] && rawtext[i] <= (byte)Identity(0xFE) && (((byte)Identity(0x80) <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE)) || ((byte)0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte)0x7E)))
                        {
                            gbchars++;
                            totalfreq += 500;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int HZProbability(byte[] rawtext)
            {
                int i, rawtextlen;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int BIG5Probability(byte[] rawtext)
            {
                int i, rawtextlen = 0;
                ...
                for (i = 0; i < rawtextlen - 1; i++)
                {
                    ...
                    {
                        dbchars++;
                        if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <= (byte)Identity(0xF9) && (((byte)0x40 <= rawtext[i + 1] && rawtext[i + 1] <= (byte)0x7E) || ((byte)Identity(0xA1) <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))))
                        {
                            bfchars++;
                            totalfreq += 500;
                            row = rawtext[i] + 256 - 0xA1;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int ENCTWProbability(byte[] rawtext)
            {
                int i, rawtextlen = 0;
                ...
                for (i = 0; i < rawtextlen - 1; i++)
                {
                    ...
                    else
                    {
                        // high bit set
                        dbchars++;
                        if (i + 3 < rawtextlen && (byte)Identity(0x8E) == rawtext[i] && (byte)Identity(0xA1) <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xB0) && (byte)Identity(0xA1) <= rawtext[i + 2] && rawtext[i + 2] <= (byte)Identity(0xFE) && (byte)Identity(0xA1) <= rawtext[i + 3] && rawtext[i + 3] <= (byte)Identity(0xFE))
                        {
                            // Planes 1 - 16
                        ...
                        else if ((byte)Identity(0xA1) <= rawtext[i] && rawtext[i] <= (byte)Identity(0xFE) && (byte)Identity(0xA1) <= rawtext[i + 1] && rawtext[i + 1] <= (byte)Identity(0xFE))
                        {
                            cnschars++;
                            totalfreq += 500;
                            row = rawtext[i] + 256 - 0xA1;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int ISO2022CNProbability(byte[] rawtext)
            {
                int i, rawtextlen = 0;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int UTF8Probability(byte[] rawtext)
            {
                int score = 0;
                int i, rawtextlen = 0;
            ...
            /// <param name="rawtext">要判断的 <see cref="byte"/> 字节数组</param>
            /// <returns>返回 0 至 100 之间的可能性</returns>
            private static int UnicodeProbability(byte[] rawtext)
            {
                //int score = 0;
                //int i, rawtextlen = 0;
                //int goodbytes = 0, asciibytes = 0;

                if (((byte)Identity(0xFE) == rawtext[0] && (byte)Identity(0xFF) == rawtext[1]) || ((byte)Identity(0xFF) == rawtext[0] && (byte)Identity(0xFE) == rawtext[1]))
                {
                    return 100;
                }
            ...
            #region ToByteArray.....
            /// <summary>
            /// 将此实例中的指定 <see cref="byte"/> 字符数组转换到 <see cref="byte"/> 字符数组。
            /// </summary>
            /// <param name="sbyteArray">要转换的 <see cref="byte"/> 字符数组</param>
            ...
            {
                byte[] sbyteArray = new byte[byteArray.Length];
            ...
            public static int ReadInput(System.IO.Stream sourceStream, ref byte[] target, int start, int count)
            {
            ...
            /// <exception cref="NotSupportedException">流不支持读取。</exception>
            /// <exception cref="ObjectDisposedException">在流关闭后调用方法。</exception>
            public static int ReadInput(System.IO.TextReader sourceTextReader, ref byte[] target, int start, int count)
            {
                // Returns 0 bytes if not enough space in target
